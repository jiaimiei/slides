...
#[try_fn]
pub fn transcode(input: impl AsRef<Path>, output: impl AsRef<Path>) -> Result<()> {
	ffmpeg::init()?;

	let mut ictx = format::input(&input)?;
	let mut octx = format::output(&output)?;
	let mut transcoder = transcoder(&mut ictx, &mut octx, &output, "anull")?;
